...
<body>
  ...
  <p>輸入參數後，按下「開始分析」即可得到和 CLI 一致的 JSON 結果。
  若市場選台股，輸入純數字代號（例如 2330）會自動轉成 2330.TW。</p>

  <div class=\"grid\">
    <div><label>股票代號（Symbol）</label><input id=\"symbol\" value=\"AAPL\" placeholder=\"例如：AAPL 或 2330\" /></div>
    <div><label>觀察區間（Lookback）</label><select id=\"lookback\"><option>20</option><option selected>60</option><option>120</option><option>240</option></select></div>
    <div><label>篩選嚴格度（Mode）</label><select id=\"mode\"><option>loose</option><option selected>standard</option><option>strict</option></select></div>
    <div><label>策略（Strategy）</label><select id=\"strategy\"><option selected>risk_reward</option><option>momentum</option></select></div>
    <div><label>分析日期（選填 YYYY-MM-DD）</label><input id=\"analysis_date\" placeholder=\"2026-04-30\" /></div>
    <div><label>市場</label><select id=\"market\"><option value=\"US\" selected>美股（US）</option><option value=\"TW\">台股（TW，自動補 .TW）</option></select></div>
    <div><label>交易週期（Trade Horizon）</label><select id=\"trade_horizon\"><option value=\"short\">短線（短停損）</option><option value=\"swing\" selected>波段（預設）</option><option value=\"position\">中長線（寬停損）</option></select></div>
    <div><label>停損方式（Stop Method）</label><select id=\"stop_method\"><option value=\"structure\">結構停損（前低）</option><option value=\"volatility\">波動停損（ATR）</option><option value=\"hybrid\" selected>混合（系統自選）</option></select></div>
  </div>
...
def normalize_symbol(raw_symbol: str, market: str) -> str:
    ...
    if market == "TW" and "." not in symbol and symbol.isdigit():
        return f"{symbol}.TW"
    if market == "US":
        return symbol
    return symbol
...
def run_analysis(payload: dict) -> dict:
    ...
    symbol = normalize_symbol(payload.get("symbol"), market)
    ...
    if market not in {"US", "TW"}:
        raise ValueError("market 不在允許值內，僅允許 US/TW")
    if not symbol:
